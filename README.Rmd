...
univariate_summary(MTPL2, x = "area", nclaims = "nclaims", exposure = "exposure") 
...
MTPL2 |>
  ...
  univariate_summary(x = "area", nclaims = "nclaims", exposure = "exposure", by = "year") |>
...
univariate_summary(MTPL2, x = "area", nclaims = "nclaims") |>
  autoplot(show_plots = 8, coord_flip = TRUE, sort = TRUE)

```

In addition to `univariate_summary()`, another option for one-way analysis is `outlier_histogram()`. This function allows you to create a histogram for continuous variables:

```{r hist1, eval = TRUE, message = FALSE, warning = FALSE}
#| fig.alt: >
#|   Histogram for the distribution of the premium

outlier_histogram(MTPL2, premium, bins = 20)
...
outlier_histogram(MTPL2, premium, bins = 10, right = 110)
...
`riskfactor_gam()` below displays the claim frequency (i.e. number of claims / exposure) for different age groups:
...
autoplot(age_policyholder_frequency, show_observations = TRUE) /
  outlier_histogram(MTPL, age_policyholder, bins = 20)
...
You can follow the same steps for severity by specifying `model = "severity"` in `riskfactor_gam()`.
...
rating_table(mod_freq1, mod_freq2) 
...
rating_table(mod_freq1, mod_freq2, model_data = dat) |>
  autoplot()
...
rating_table(mod_freq1, mod_freq2, model_data = dat, exposure = exposure) |>
...
rating_table(burn_unrestricted, model_data = MTPL_premium, exposure = exposure) |>
...
burn_unrestricted |>
  add_smoothing(x_cut = "age_policyholder_freq_man", 
...
# Show rating factors
rating_table(burn_restricted3) |> autoplot()
...
premiums3 <- extract_model_data(burn_restricted3) |>
...
premiums4 <- extract_model_data(burn_restricted3) |>
