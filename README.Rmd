...
Two common methods for compactly displaying numerical data across a wide range of values are by applying a logarithmic transformation or excluding the outliers. However, both methods obscure the original distribution. An alternative approach is to group all outliers into a single bin, which preserves the original distribution while also providing insight into the number of outliers.
...
This section shows how to fit a GAM (Generalized Additive Model) for the variable *age_policyholder* in the MTPL data set: 
...
In this example, *exposure* refers to the amount of risk being insured. In this case, an insured vehicle counts as an exposure. If the vehicle is insured starting from July 1 of a given year, it would represent an exposure of 0.5 to the insurance company for that year.

The figure indicates that younger policyholders have a higher risk profile. For policyholders aged 19, the fitted GAM is lower than expected based on the observed claim frequency. This is due to the small number of 19-year-old policyholders in the portfolio.
...
dat <- MTPL |>
  ...
  mutate(age_policyholder_freq_catm = cut(age_policyholder, 
...
mod_freqm <- glm(nclaims ~ age_policyholder_freq_catm, 
                 offset = log(exposure), 
                 family = "poisson", 
                 data = dat)
...
MTPL_premium <- dat |>
  add_prediction(mod_freq, mod_freqm, mod_sev) |>
  mutate(premium = pred_nclaims_mod_freq * pred_amount_mod_sev) |>
  mutate(premiumm = pred_nclaims_mod_freqm * pred_amount_mod_sev)
...
burn_unrestrictedm <- glm(premium ~ zip + bm + age_policyholder_freq_catm, 
                         weights = exposure, 
                         family = Gamma(link = "log"), 
                         data = MTPL_premium) 
...
burn_unrestrictedm |>
  smooth_coef(x_cut = "age_policyholder_freq_catm", 
              x_org = "age_policyholder", 
              smoothing = "gam",
              breaks = seq(18, 95, 5)) |>
  autoplot()
...
Other options include scams, make list:

1. mpd
2. mpi

```{r example18a1, eval = TRUE, message = FALSE, warning = FALSE}
...
Or with a gam weighted:

```{r example18a2, eval = TRUE, message = FALSE, warning = FALSE}
...
burn_restricted3m <- burn_unrestrictedm |>
  restrict_coef(restrictions = zip_df) |>
  smooth_coef(x_cut = "age_policyholder_freq_catm", 
              x_org = "age_policyholder", 
              breaks = seq(18, 95, 5)) |> 
  update_glm(intercept_only = TRUE) 

# Show rating factors
rating_factors(burn_restricted3m)
rating_factors(burn_unrestrictedm)
