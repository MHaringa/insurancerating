...
### Tariff segmentation (`derive_tariff_segments()` / `construct_tariff_classes()`)
- `derive_tariff_segments()` is the primary API for deriving tariff segments
  from a fitted `risk_factor_gam()` object and returns objects with primary
  class `"tariff_segments"`.
- `construct_tariff_classes()` remains available as a deprecated compatibility
  wrapper for the old API.
