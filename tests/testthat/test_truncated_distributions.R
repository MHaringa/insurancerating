...
test_that("fit_truncated_severity fits gamma severity with new argument names", {
  set.seed(2)
  ...
  expect_s3_class(fit, "truncated_severity")
...
test_that("fit_truncated_severity validates core inputs", {
  expect_error(
    fit_truncated_severity(losses = character(), show_summary = FALSE),
    "`losses` must be a non-empty numeric vector.",
    fixed = TRUE
  )
  expect_error(
    fit_truncated_severity(losses = c(NA_real_, Inf), show_summary = FALSE),
  ...
  expect_error(
    fit_truncated_severity(losses = c(100, 200), n_shape_grid = 0,
                       show_summary = FALSE),
    "`n_shape_grid` must be a positive whole number.",
    fixed = TRUE
  )
  expect_error(
    fit_truncated_severity(losses = c(100, 200), show_progress = NA,
                       show_summary = FALSE),
...
test_that("fit_truncated_severity validates start values", {
...
test_that("deprecated fit_truncated_severity argument names still work", {
  set.seed(3)
...
test_that("fit_truncated_dist remains available as deprecated alias", {
  set.seed(5)
  losses <- rgammat(30, shape = 2, scale = 1000, lower = 500, upper = 5000)
...
test_that("autoplot.truncated_severity supports new and deprecated arguments", {
  set.seed(4)
...
test_that("autoplot.truncated_severity validates arguments", {
  ...
  expect_error(
    insurancerating:::autoplot.truncated_severity(object),
