...
test_that("error when vec not numeric", {
  x <- letters[1:10]
  expect_error(fisher_classify(x), "`vec` must be numeric")
})

test_that("error when vec shorter than n", {
  x <- 1:5
  expect_error(fisher_classify(x, n = 10), "longer than number of classes")
})
...
test_that("same result when rerun on same data (deterministic)", {
  ...
  result1 <- fisher_classify(x, n = 4)
  result2 <- fisher_classify(x, n = 4)
