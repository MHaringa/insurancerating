...
vignette: >
  %\VignetteIndexEntry{Pricing principles}
...
Insurance pricing is not only a statistical modelling exercise.
It is the process of translating observed experience into a tariff that is:
...
This vignette outlines key concepts that are commonly used with the building
blocks in `insurancerating`.
...
The goal is not necessarily to improve statistical fit, but to support a tariff
structure that is:
...
The goal is not necessarily to “improve the model”, but to support a tariff
structure where:
...
No single model optimises all dimensions. A useful pricing framework makes these
trade-offs:
