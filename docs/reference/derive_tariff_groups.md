...
Derives data-driven tariff groups for a continuous risk factor from a
...
The groups help translate a smooth GAM response pattern into practical
...
A `list` of class `"tariff_groups"` with components:
...
- classification_data:
  ...
  Data frame used to derive the groups.
...
- class_boundaries:
  ...
  Numeric vector with group boundaries.

- assigned_groups:

  Factor with the tariff group assigned to each observed risk factor
  value.

For backward compatibility, the old components `prediction`, `x`,
`model`, `data`, `x_obs`, `splits`, and `tariff_classes` are also
returned.
...
groups. This method is based on the work by Henckaerts et al. (2018).
