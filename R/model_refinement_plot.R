...
.plot_restriction_step <- function(state,
  ...
  if (identical(step$type, "rebasing")) {
    values <- state$rebasing_df
    if (is.null(values) || !all(c(
      "level", "original_relativity", "rebased_relativity"
    ) %in% names(values))) {
      stop("No rebasing plot data found for this step.", call. = FALSE)
    }
    ...
    plot_data <- rbind(
      ...
      data.frame(
        level = values$level,
        type = "Rebased relativity",
        Coef = values$rebased_relativity,
        stringsAsFactors = FALSE
      )
    )
    level_order <- unique(as.character(values$level))
    ...
    p <- ggplot2::ggplot(
      ...
      ggplot2::scale_colour_manual(
        values = c(
          "Current relativity" = pal$frequency,
          "Rebased relativity" = pal$risk_premium
        ),
        name = NULL
      ) +
      ggplot2::labs(x = x_lab, y = "Relativity") +
