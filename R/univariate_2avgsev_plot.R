#' Automatically create a ggplot for objects obtained from univariate_average_severity() (deprecated function; use 'autoplot.univariate()' instead)
#' @noRd
#'
...
autoplot.univ_avgsev <- function(x, background = TRUE, labels = TRUE, sort = FALSE, sort_manual = NULL, dec.mark = ",", color = "dodgerblue", color_bg = "#E7B800", label_width = 10){
  ...
  if ( isTRUE ( background )){
    ...
    hist_bg <- list(
      ggplot2::geom_bar(data = df, aes(x = .data[[xvar]], y = .data[["nclaims_scale"]]),
                     stat = "identity", color = color_bg, fill = color_bg, alpha = 0.4),
      ggplot2::scale_y_continuous(sec.axis = sec_axis(~ . * max(df[[nclaims]]) / max(df$average_severity),
                                             name = nclaims,
                                             labels = sep_fn),
                         labels = sep_fn,
                         limits = c(0, NA),
  ...
  if (isTRUE( background ) & isTRUE( labels )) {
    labels_bg <- list(
      ggplot2::geom_text(data = df, aes(x = .data[[xvar]], y = .data[["nclaims_scale"]], label = sep_fn(.data[[nclaims]])),
                vjust = "inward", size = 3)
    )
  }
  ...
  p1 <- ggplot2::ggplot(data = df) +
    ...
    ggplot2::geom_point(aes(x = .data[[xvar]], y = .data[["average_severity"]]), color = color) +
    ggplot2::geom_line(aes(x = .data[[xvar]], y = .data[["average_severity"]], group = 1), color = color) +
