...
print.check_residuals <- function(x, ...) {
  p.val <- x$p.val
  if (p.val < 0.05) {
    insight::print_color(sprintf("Warning: deviations from the expected distribution detected (p = %.3f).", p.val), "red")
  } else {
    insight::print_color(sprintf("OK: residuals appear as from the expected distribution (p = %.3f).", p.val), "green")
  }
}
