...
#'
#' @description Constructs insurance tariff classes to `fitgam` objects produced
#' by `fit_gam`. The goal is to bin the continuous risk factors such that
#' categorical risk factors result which capture the effect of the covariate on
#' the response in an accurate way, while being easy to use in a generalized
#' linear model (GLM).
#'
#' @param object fitgam object produced by `fit_gam`
#' @param alpha complexity parameter. The complexity parameter (alpha) is used
#' to control the number of tariff classes. Higher values for `alpha` render
#' less tariff classes. (`alpha` = 0 is default).
...
#' reproducibility).
#'
#' @details Evolutionary trees are used as a technique to bin the `fitgam`
#' object produced by `fit_gam` into risk homogeneous categories. This method
#' is based on the work by Henckaerts et al. (2018). See Grubinger et al. (2014)
#' for more details on the various parameters that control aspects of the
#' evtree fit.
#'
...
#' \item{tariff_classes}{values in vector `x` coded according to which
...
#' construct_tariff_classes()
#'
#' @description Takes an object produced by `construct_tariff_classes()`, and
#' plots the fitted GAM. In addition the constructed tariff classes are shown.
#'
...
#'
#' @export
autoplot.constructtariffclasses <- function(object, conf_int = FALSE,
                                            color_gam = "steelblue",
  ...
  gam_plot <- ggplot(data = prediction, aes(x = x, y = predicted)) +
    ...
    { if(isTRUE(show_observations) & ylab == "frequency")
      geom_point(data = points, aes(x = x, y = frequency),
    ...
    { if(isTRUE(rotate_labels)) theme(axis.text.x =
