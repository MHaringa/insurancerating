...
#'
#' @export
autoplot.univ_lossratio <- function(x, background = TRUE, labels = TRUE, sort = FALSE, dec.mark = ",", color = "dodgerblue", color_bg = "#E7B800"){
  ...
  if ( isTRUE ( background )){
    ...
    hist_bg <- list(
      geom_bar(data = df, aes(x = .data[[xvar]], y = .data[["premium_scale"]]),
                     stat = "identity", color = color_bg, fill = color_bg, alpha = 0.4),
      scale_y_continuous(sec.axis = sec_axis(~ . * max(df[[premium]]) / max(df$loss_ratio),
                                             name = premium,
                                             labels = sep_fn),
                         labels = sep_fn,
                         limits = c(0, NA),
  ...
  if (isTRUE( background ) & isTRUE( labels )) {
    labels_bg <- list(
      geom_text(data = df, aes(x = .data[[xvar]], y = .data[["premium_scale"]], label = sep_fn(.data[[premium]])),
                vjust = "inward", size = 3)
    )
  }
  ...
  p1 <- ggplot(data = df) +
    ...
    geom_point(aes(x = .data[[xvar]], y = .data[["loss_ratio"]]), color = color) +
    geom_line(aes(x = .data[[xvar]], y = .data[["loss_ratio"]], group = 1), color = color) +
    {if( isTRUE( background ) & isTRUE( labels )) labels_bg} +
    theme_minimal() +
    labs(y = "Loss ratio", x = xvar) +
